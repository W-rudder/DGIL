...
parser.add_argument('--eval_neg_samples', type=int, default=0, help='how many negative samples to use at inference. Note: this will change the metric of test set to AP+AUC to AP+MRR!')
...
df['label'] = ldf['label'].astype(np.int64)
...
if 'combine_neighs' in train_param and train_param['combine_neighs']:
    combine_first = True
model = GeneralClsModel(gnn_dim_node, gnn_dim_edge, sample_param, memory_param, gnn_param, train_param, combined=combine_first).to(device)
...
def eval(mode='val'):
    ...
    with torch.no_grad():
        ...
        for _, rows in eval_df.groupby(eval_df.index // train_param['batch_size']):
            root_nodes = np.concatenate([rows.src.values]).astype(np.int32)
            ts = np.tile(rows.time.values, 1).astype(np.float32)
            ...
            aps.append(f1_score(label, torch.argmax(pred, dim=1), average="micro"))
            if neg_samples > 1:
                pass
                ...
                aucs_mrrs.append(roc_auc_score(label, pred[:, 1]))
            if mailbox is not None:
...
for e in range(train_param['epoch']):
    ...
    for _, rows in df[:train_edge_end].groupby(group_indexes[random.randint(0, len(group_indexes) - 1)]):
        ...
        ts = np.concatenate([rows.time.values]).astype(np.float32)
        if sampler is not None:
            if 'no_neg' in sample_param and sample_param['no_neg']:
