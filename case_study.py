...
parser.add_argument('--nc_model', type=str, default='', help='name of stored nc_model to load')
...
ldf = pd.read_csv('DATA/{}/case_labels.csv'.format(args.data))
...
print('ori c:', c)
model = NodeClassificationModel(emb.shape[1], args.dim, labels.max() + 1, c)
if args.nc_model:
    model.load_state_dict(torch.load(args.nc_model, map_location=torch.device('cpu')))
